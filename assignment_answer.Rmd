...
If managers do not respond at all, customers won't get the idea of being noticed or heard by the firm. If customers have the feeling that they are not heard by the firm, they will not take time to write positive reviews, or feedback, since they will feel that it is a waste of their time. Also, managers of firms who respond a lot to either positive or negative reviews will increase the cost of leaving negative reviews by the customers, since they know their comments will be read.
...
```{r}
hotels_orm <- hotels_orm %>%
  filter(xplatform_dd_obs == 1) %>%
  select(hotel_id, year, stars, after, ta_dummy, first_response, cum_avg_stars_lag, log_count_reviews_lag, ta_dummy, t, ash_interval, traveler_type)
```
...
The 'Ashenfelter dip' is named after Ashenfelter who discovered a (temporary) dip in the wage of participants before entering a job training program. This could be a problem for his research, because lower pre-programme average could mean that the DD estimates are exaggerated (Ashenfelter and Card, 1985). Proserpio & Zervas believe they see a partin akin to an Ashenfelter dip in their application, because they detected a drop in the TripAdvisor ratings 30 days before the start of the treatment period. 

6. Let's compute the 'simple' Difference in Difference estimate using differences in means.
  (a) Create a data frame with two rows and two columns where the rows take the values of first_response = 0 or 1, and the columns take the values of ta_dummy = 0 or 1. 
...
```{r}
# a
simple_did <- hotels_orm[!is.na(hotels_orm$cum_avg_stars_lag),]

simple_did <- simple_did %>%
  group_by(ta_dummy, first_response) %>%
  summarize(cum_avg_stars_lag=mean(cum_avg_stars_lag)) %>%
  ungroup()

simple_did_a <- simple_did %>%
  pivot_wider(names_from = ta_dummy,
              values_from = cum_avg_stars_lag)

# b
bef_aft_exp <- filter(simple_did, ta_dummy == '0', first_response == '1' )$cum_avg_stars_lag - filter(simple_did, ta_dummy == '0', first_response == '0' )$cum_avg_stars_lag

bef_aft_ta <-  filter(simple_did, ta_dummy == '1', first_response == '1' )$cum_avg_stars_lag - filter(simple_did, ta_dummy == '1', first_response == '0' )$cum_avg_stars_lag

# c
did <- bef_aft_ta - bef_aft_exp

print(paste("Diff in Diff Estimate: ", did))

## versie 2 vraag 6 
# simple_did_a <- rename(simple_did_a, first_response_0 = "0", first_response_1 = "1")
# simple_did_b <- simple_did_a %>%
#  mutate(difference = first_response_1 - first_response_0)
  
# simple_did_c <- diff(simple_did_b$difference, lag = 1)
# print(simple_did_c)
...
```{r}
hotels_orm_tt <- hotels_orm %>%
  filter(year >= 2009, ash_interval == 0)
hotels_orm_tt <- hotels_orm_tt[!is.na(hotels_orm_tt$traveler_type),]  
```
...
Ashenfelter OC, Card D (1985) Using the longitudinal structure of
earnings to estimate the effect of training programs. Rev. Econom.
Statist. 67(4):648–660.
